...
class WeightTest extends TestCase
{
    ...
    public function testItInstallsNothingOfItsOwn(): void
    {
        foreach (array_keys($this->composer()['require']) as $package) {
            $this->assertTrue(
                $package === 'php' || str_starts_with($package, 'illuminate/'),
                "$package would be installed in every application that uses this package"
            );
        }
    }
    ...
    public function testItIsSmallEnoughToRead(): void
    {
        ...
        $this->assertLessThan(1200, $lines, "$lines lines in src/, the limit is 1200");
    }
