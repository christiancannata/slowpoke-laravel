...
use Slowpoke\Laravel\Tests\Fixtures\ThrowingSender;
...
class TracerTest extends TestCase
{
    ...
    public function testDriverNamesFollowOpenTelemetry(): void
    {
        $tracer = $this->tracer();
        $tracer->startRequest('GET', $this->now);
        foreach (['pgsql', 'mariadb', 'sqlite', 'sqlsrv', 'oracle'] as $driver) {
            $tracer->recordQuery('select 1 from t', 1.0, $driver);
        }
        $tracer->finishRequest('a', '/a', 200);
        $tracer->flush();
        [, $queries] = $this->sender->onlyTrace();
        $this->assertSame(['postgresql', 'mariadb', 'sqlite', 'microsoft.sql_server', 'oracle'], array_map(function ($q) {
    ...
    public function testJobTrace(): void
    {
        ...
        $this->assertSame('App\\Jobs\\SendInvoices', $root['name']);
    ...
    {
        $tracer = $this->tracer();
        $tracer->startJob('App\\Jobs\\Flaky', 'default');
    ...
    public function testAJobInsideARequestBelongsToTheRequest(): void
    {
        $tracer = $this->tracer();
        $tracer->startRequest('GET', $this->now);
        $tracer->startJob('App\\Jobs\\Inline', 'default');
    ...
    {
        // Long-running servers (Octane) keep the first request's time in $_SERVER.
        $tracer = $this->tracer();
